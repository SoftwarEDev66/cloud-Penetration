...
#requests.packagrs.urllib3.disable_warninh(InsecureRequestWarning)
...
try:
	from urlparse import urlparse
		...
		import cfscrape
			...
			print('\nunable to install the cfscrape module by pip. this tool requires cfscrape to run. You Can get it here by: https://github.com/Anorov/clpudfare-scrape \n')
			sys.exit(1)
			Intro = ' ' '\n
\033[44m\033[34m========================\033[0m			
\033[40m\33[34m=========cloud Penetration v1.0=========\033[0m
\033[44m\033[34====Authour:SoftWarEDev66===========\033[0m
]\003[44m\033[34m=======================\033[0m
\033[40m\033[34;2m========Follow Me On GitHub========\033[0m
\033[40m\033[34m======Team:TermuxHackz, CyberneticFreaks,DedSec=======\033[0m

 print(intro)
 try: 
 from bs4 import BeautifulSoup, UnicodeDamnit
 except:
 	print('BeautifulSoup is required to run the script.')
 	sys.exit(1)
 	    global cfurl
 	    global usecurl
 	    global writeout
 	    ...
 	    outpath = 'download'
 	    finished =[ ]
 	    parser = argparse.ArgumentParser(description="a script to automatically bypass anti-robot measures, scrape for linls, and automate downloading multiple files from server behind a cloudfare proxy")
...
parser.add_argument('--version', action='version', version='%(prog)s v0.78 by SoftWarEDev66<SoftWarEDev66@eudemonics.org>, released December 6, 2021.')
...
unithr = "hours"
         ...
         while si < slen:
            for slink in sfindlinks:
               ...
               if sl:
                  ...
                  print('\n\033[34;1mNO LINKS FOUND at: \033[32;21m%s \033[0m\n' % str(slink))
                  si += 1
                  continue

         for sd in sdirs:
            if '/' in sd[-1:]:
               print('\n\033[33mfollowing directory: \033[35m%s \033[0m\n' % sd)
               followlinks(sd)
               getCF(sd, 1)
            else:
               print('\n\033[31mrequesting link: \033[32m%s \033[0m\n' % sd)
               getCF(sd, 0)
   ...
   if links == 1:
      ...
      while found > 0 and keep != 0:
         ...
         if re.match(matchtop, cfurl) and len(finished) > 0:
            keep = 0
